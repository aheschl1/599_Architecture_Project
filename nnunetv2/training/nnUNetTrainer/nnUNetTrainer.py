...
from nnunetv2.training.loss.compound_losses import DC_and_CE_loss, DC_and_BCE_loss, DC, CE_loss, DC_and_CE_SURFACE_loss
...
class nnUNetTrainer(object):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
                 device: torch.device = torch.device('cuda'), epochs=1000, zero_grad_iters=1):
        ...
        self.zero_grad_iters = zero_grad_iters
    ...
    def initialize(self):
        if not self.was_initialized:
            ...
            self.print_to_log_file(f"Model parameters: {sum(p.numel() for p in self.network.parameters())}")
    ...
    def _build_loss(self):
        ...
        if self.configuration_manager.loss_function != None:
            if self.configuration_manager.loss_function == 'CE_loss':
                loss = CE_loss({},
                ignore_label=self.label_manager.ignore_label)
            elif self.configuration_manager.loss_function == 'DC':
                loss = DC(
                    {'batch_dice': self.configuration_manager.batch_dice,'smooth': 1e-5, 'do_bg': False, 'ddp': self.is_ddp},
                )
            else:
                assert False, "Invalid loss in configuration"
        else:
        ...
        # loss = DC_and_CE_SURFACE_loss({'batch_dice': self.configuration_manager.batch_dice,
        #                            'smooth': 1e-5, 'do_bg': False, 'ddp': self.is_ddp}, {}, weight_ce=1, weight_dice=1,
        #                           ignore_label=self.label_manager.ignore_label, dice_class=MemoryEfficientSoftDiceLoss)
    ...
    def train_step(self, batch: dict, batch_id:int=0) -> dict:
        ...
        if batch_id == 0 or (batch_id-1) % self.zero_grad_iters == 0:
            self.optimizer.zero_grad()
        ...
        if self.grad_scaler is not None:
            ...
            if batch_id % self.zero_grad_iters == 0:
