...
def get_trainer_from_args(dataset_name_or_id: Union[int, str],
    ...
    nnunet_trainer = nnunet_trainer(plans=plans, configuration=configuration, fold=fold,
                                    dataset_json=dataset_json, unpack_dataset=not use_compressed, device=device, epochs=epochs, zero_grad_iters=zero_grad_iters)
    
    try:
        nnunet_trainer.set_model_path(model_path)
    except:
        pass
...
def run_ddp(rank, dataset_name_or_id, configuration, fold, tr, p, use_compressed, disable_checkpointing, c, val, pretrained_weights, npz, world_size, model_path, epochs, zero_grad_iters):
    setup_ddp(rank, world_size)
    torch.cuda.set_device(torch.device('cuda', dist.get_rank()))

    nnunet_trainer = get_trainer_from_args(dataset_name_or_id, configuration, fold, tr, p,
                                           use_compressed, model_path=model_path, epochs=epochs, zero_grad_iters=zero_grad_iters)
    ...
    if num_gpus > 1:
        ...
        mp.spawn(run_ddp,
                 args=(
                     ...
                     num_gpus, model_path, epochs, zero_grad_iters),
                 nprocs=num_gpus,
                 join=True)
    else:
        nnunet_trainer = get_trainer_from_args(dataset_name_or_id, configuration, fold, trainer_class_name,
                                               plans_identifier, use_compressed_data, device=device, model_path=model_path, epochs=epochs, zero_grad_iters=zero_grad_iters)
...
def run_training_entry():
    ...
    parser.add_argument('-model', required=False, type=str, help="The path to the json model definition.")
    parser.add_argument('-epochs', required=False, type=int, default=1000, help="How many epochs to run.")
    parser.add_argument('-zero_grad_iters', required=False, type=int, default=1, help="How many epochs until backwards update.")
    parser.add_argument('--monai_loss', required=False, action="store_true", help="Default loss instead of monai.")
    ...
    if args.monai_loss:
        ...
        ourTrainer.loss_mode = "monai"

    run_training(args.dataset_name_or_id, args.configuration, args.fold, args.tr, args.p, args.pretrained_weights,
                 args.num_gpus, args.use_compressed, args.npz, args.c, args.val, args.disable_checkpointing,
                 device=device, model_path=args.model, epochs=args.epochs, zero_grad_iters=args.zero_grad_iters)
