...
def convert_predicted_logits_to_segmentation_with_correct_shape(predicted_logits: Union[torch.Tensor, np.ndarray],
    ...
    if numpy_type and isinstance(segmentation, torch.Tensor):
        segmentation = segmentation.cpu().numpy()

    # put segmentation in bbox (revert cropping)
    library = np if numpy_type else torch
    segmentation_reverted_cropping = library.zeros(properties_dict['shape_before_cropping'],
                                              dtype=library.uint8 if len(label_manager.foreground_labels) < 255 else library.uint16)
    ...
    if return_probabilities:
        ...
        if numpy_type:
            ...
            predicted_probabilities = predicted_probabilities.permute([0] + [i + 1 for i in
                                                                           plans_manager.transpose_backward])
